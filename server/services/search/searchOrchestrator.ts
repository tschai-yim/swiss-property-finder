...
import {
  getActiveProviders,
  createRequestManagers,
  mergeProviderStreams,
} from "./providerService";
import { PropertyWithoutCommuteTimes, SearchContext } from "../providers/providerTypes";
...
export async function* streamProperties(
  ...
  if (!searchAreaResult) {
    yield {
      type: "progress",
      message: "Error: Failed to determine search area.",
    };
    return;
  }
  const {
  ...
  if (activeProviders.length === 0) {
    yield {
      type: "progress",
      message: "No property providers are enabled. Search complete.",
    };
    return;
  }
  const requestManagers = createRequestManagers(activeProviders, debugConfig);
  const { buckets, exclusionKeywords, genderPreference, rentalDuration } =
    filters;
  ...
  excludedProperties.forEach((p) =>
    exclusionSet.addForLookupOnly(p as Property)
  );
  ...
  yield {
    type: "progress",
    message: `Querying ${places.length} location(s) with ${activeProviders.length} provider(s)...`,
  };
  ...
  for await (const propertyBatch of mergedProviderStream) {
    ...
    for (let i = 0; i < propertyBatch.length; i += BATCH_SIZE) {
      ...
      const nonExcludedBatch = chunk.filter(
        (prop) => !exclusionSet.findDuplicate(prop)
      );

      // Step 3b: De-duplicate and merge the remaining incoming properties.
      const mergedBatch = nonExcludedBatch.map(
        (prop) => propertySet.add(prop).finalProperty
      );
      ...
      if (destCoords && isochrones.length > 0) {
        isochroneFilteredBatch = mergedBatch.filter((p) =>
          isochrones.some((iso) =>
            isPointInPolygon({ lat: p.lat, lng: p.lng }, iso.polygon)
          )
        );
      }
      ...
      if (
        isochroneFilteredBatch.length > 0 &&
        destCoords &&
        modesToEnrich.length > 0
      ) {
        processedCount += isochroneFilteredBatch.length;
        yield {
          type: "progress",
          message: `Enriching ${processedCount}+ properties...`,
        };
        const enrichedBatch = (
          await enrichItemsWithTravelTimes(
            isochroneFilteredBatch,
            destCoords,
            modesToEnrich,
