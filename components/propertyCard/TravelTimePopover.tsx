...
import { Property, FilterCriteria, TravelMode } from '../../types';
...
export const TravelTimePopover: React.FC<TravelTimePopoverProps> = ({ property, travelModes }) => {
    const allModes: TravelMode[] = ['public', 'bike', 'car', 'walk'];
    const bestTimeInfo = getBestTravelTime(property, travelModes);

    const travelTimeMap: Record<TravelMode, { time: number | null | undefined, icon: React.FC<{className?: string}>, name: string }> = {
        public: { time: property.commuteTimes.public, icon: TrainIcon, name: "Transit" },
        bike: { time: property.commuteTimes.bike, icon: BikeIcon, name: "Bike" },
        car: { time: property.commuteTimes.car, icon: CarIcon, name: "Car" },
        walk: { time: property.commuteTimes.walk, icon: WalkIcon, name: "Walk" },
    };
    ...
    return (
        <div className={containerClasses}>
            <ul className="space-y-1">
                {allModes.map(mode => {
                    const { icon: Icon, time, name } = travelTimeMap[mode];
